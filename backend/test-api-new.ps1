Write-Host "`n🧪 Mobile Shop API Test Suite" -ForegroundColor Cyan
...
try {
    $r = Invoke-RestMethod -Uri "http://localhost:5000/api/auth/seed-admin" -Method Post
...
try {
    $login = Invoke-RestMethod -Uri "http://localhost:5000/api/auth/staff/login" -Method Post -Body $body -ContentType "application/json"
    ...
    $profile = Invoke-RestMethod -Uri "http://localhost:5000/api/auth/profile" -Method Get -Headers @{Authorization = "Bearer $token"}
